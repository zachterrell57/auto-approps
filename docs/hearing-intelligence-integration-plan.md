...
The MVP delivers a coherent local module: create a hearing job from URL with an optional client, resolve the committee page's YouTube video, capture/transcribe audio chunks, configure watchlists, detect hits, generate cited outputs after capture stops, review/edit/verify/comment, export memo packages, and run an evaluation harness. Production hardening requires the listed platform capabilities.
